...
import { CompanyLeads } from "../../types";
...
function ActionsCell({ row }: { row: Row<CompanyLeads> }) {
  const { copy } = useCopyToClipboard();
  const handleCopyId = () => {
    copy(row.original.id);
    const message = `Company Lead ID successfully copied: ${row.original.id}`;
    toast.custom(
      () => (
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent side="bottom" align="end">
        ...
        <DropdownMenuItem onClick={() => {}}>Edit Campaign</DropdownMenuItem>
...
interface CompanyLeadsTableProps {
  data: CompanyLeads[];
}

export function CompanyLeadsTable({ data }: CompanyLeadsTableProps) {
  ...
  const getLeadStageBadge = (stage: CompanyLeads["leadStage"]) => {
    switch (stage) {
      case "Pending":
  ...
  const columns = useMemo<ColumnDef<CompanyLeads>[]>(
    () => [
      {
        accessorKey: "id",
        id: "id",
            ...
            >
              ...
              <div className="space-y-px">
                ...
                <div className="text-sm text-muted-foreground">
                  {row.original.contactsCount} contacts
                </div>
              </div>
            </Link>
          );
  ...
  const table = useReactTable({
    ...
    getRowId: (row: CompanyLeads) => row.id,
    state: {
      pagination,
      sorting,
      columnOrder,
